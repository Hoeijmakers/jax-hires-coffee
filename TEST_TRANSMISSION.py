...
if not batch:
    oi = 3 #This is the order that's going to be plotted below.
...
true_downscoped_model = model_jax(true_p,wld,wli,kappa_grid,x_kernel,jnp.array(phase),c,gamma,k,m,g,P0,R0,Rs**2,n_species)
...
DATA = fxd[0:len(phase)] * (true_downscoped_model+1)
...
def numpyro_model(y,y_e,*args):
    ...
    beta = numpyro.sample('$\\beta$', dist.Uniform(low=0.2, high=5.0))
...
# Run the MCMC
mcmc.run(rng_keys,DATA,DATA_E,wld,wli,kappa_grid, x_kernel,phase,c,gamma,k,m,g,P0,R0, Rs**2,n_species)
mcmc.print_summary()
...
#     result = arviz.from_numpyro(mcmc)
    ...
    corner(
        ...
        quiet=True,
        truths=[truths[k] for k,v in mcmc.get_samples().items()],
        labels = [k for k,v in mcmc.get_samples().items()]
    );
