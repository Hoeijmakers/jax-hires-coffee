...
def model_jax(p,wl,wlk,kappa_grid,x_kernel,filters,phase,c,gamma,k,m,g,P0,R0,Rs2,n_species):
    ...
    spec2D = jnp.interp(shifted_wl,wlk,RT_b) * c0 * filters
...
true_downscoped_model = model_jax(true_p,wld,wli,kappa_grid,x_kernel,fxf*0.0+1.0,jnp.array(phase),c,gamma,k,m,g,P0,R0,Rs**2,n_species)
#A model without filter applied.
...
DATA = fxd * fxf * true_downscoped_model
...
DATA_E = err[0:len(phase)]
DATA_E = np.array(DATA_E)
DATA_E[~np.isfinite(DATA_E)] = 5.0 # A large error.
...
t1=ut.start()
for i in range(N):
    true_downscoped_model = model_jax(true_p,wld,wli,kappa_grid,x_kernel,fxf*0.0+1.0,phase,c,gamma,k,m,g,P0,R0,Rs**2,n_species)
...
def numpyro_model(y,y_e,*args):
    ...
    numpyro.sample("obs", dist.Normal(loc=model_jax(priors,*args),
                                      scale=y), obs=y_e)
...
# Run the MCMC
mcmc.run(rng_keys,DATA,DATA_E,wld,wli,kappa_grid, x_kernel,fxf,phase,c,gamma,k,m,g,P0,R0, Rs**2,n_species)
mcmc.print_summary()
